...
Tessitura.UserModelView=Tessitura.View.extend({className:"user-model",template:JST["users/model"],events:{"dblclick span.p":"displayInput","keydown input":"triageKeypress"},displayInput:function(t){var e=t.target?$(t.target).attr("class")&&$(t.target).attr("class").match(/profile-field/)?$(t.target):$(t.target).closest(".profile-field"):$(t);e.find(".p").hide(),e.find(".input").show(),this.resizeInputs(e),e.find("input").focus().select()},submitUpdate:function(t){var e={};e[$(t.target).attr("name")]=$(t.target)[0].value,this.model.save(e,{beforeSend:function(t){t.setRequestHeader("Authorization","Basic "+$.cookie("auth"))},error:function(t,e){console.log(e)},success:function(i){if(e.username){var s=btoa(i.get("username")+":"+i.get("password"));$.removeCookie("auth"),$.cookie("auth",s,{expires:365})}$(t.target).closest("span.input").hide()}})},triageKeypress:function(t){var e=13===t.keyCode,i=9===t.keyCode,s=27===t.keyCode,a=$(t.target).attr("name"),n=$(t.target)[0].value;if(e&&(this.hideInputs(),n!==this.model.get(a)&&""!==n&&this.submitUpdate(t)),i){t.preventDefault();var r="first_name"===$(t.target).closest(".profile-field").attr("id")?this.$("#last_name"):$(t.target).closest("tr").next().find("td > span.profile-field");this.model.get(a)!==n&&""!==n&&this.submitUpdate(t),this.hideInputs(),this.displayInput(r)}s&&this.hideInputs()},updateDisplay:function(){var t=this;this.$(".profile-field").each(function(e,i){var s=$(i).attr("id"),a=t.model.get(s);$(i).find(".p").html(a||"N/A"),$(i).find("input").attr("value",a)})},hideInputs:function(){this.$("span.input").hide(),this.$("span.p").show()},resizeInputs:function(t){if(["first_name","last_name"].indexOf(t.attr("id"))>-1){var e=parseInt(t.find(".p").css("width").match(/\d*/)[0])+10;t.find("input").css("width",e+"px")}},initialize:function(){this.listenTo(this.model,"change",this.updateDisplay)},render:function(){var t=this;return Tessitura.View.prototype.render.call(t,t.template({model:t.model}))}}),module.exports=Tessitura.UserModelView;
